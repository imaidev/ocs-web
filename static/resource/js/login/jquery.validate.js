					...
					function handle() {
						...
						if (validator.settings.submitHandler) {
							if (validator.submitButton) {
								...
								// the missing submit
								// button
								hidden = $("<input type='hidden'/>").attr("name", validator.submitButton.name).val(validator.submitButton.value).appendTo(
			...
			.extend(
					$.validator,
					{
						...
						messages : {
							...
							remote : "Has been used,please re-enter.",
						...
						prototype : {
							...
							init : function() {
								...
								$(this.currentForm).validateDelegate(
										...
										"focusin focusout keyup", delegate).validateDelegate("[type='radio'], [type='checkbox'], select, option", "click",
										delegate);
							...
							elements : function() {
								...
								return $(this.currentForm).find("input, select, textarea").not(":submit, :reset, :image, [disabled]").not(this.settings.ignore)
										.filter(function() {
								...
								for ( var method in rules) {
									...
									try {
										...
										if (this.settings.debug && window.console) {
											console
													.log("exception occured when checking element " + element.id + ", check the '" + rule.method + "' method",
															e);
										}
										throw e;
									}
						...
						methods : {
							...
							equalTo : function(value, element, param) {
								...
								// var target = ${verify};
								// var num =
								// document.getElementById('checknum_per');
							...
							loginName_rule : function(value, element) {
								return this.optional(element) || /^[a-zA-Z][a-zA-Z0-9_]{4,19}$/i.test(value);
							},
							password_rule : function(value, element) {
								return this.optional(element) || /^[\w\W]{6,15}$/i.test(value);
							},

							phone_rule : function(value, element) {
								return this.optional(element) || /^13[0-9]{9}$|14[0-9]{9}$|15[0-9]{9}$|17[0-9]{9}$|18[0-9]{9}$/i.test(value);
							},
							// 同时支持手机和电话
							...
							qgmytele_rule : function(value, element) {
								return this.optional(element)
								|| /(^((\d{2,3})-)(\d{7,8})(-(\d{3,}))?$)/i.test(value);
							/*
							 * ||
							 * /((\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$)/i.test(value);
							 */
					        },
							character_test : function(value, element) {
								return this.optional(element) || /^([\u4e00-\u9fa5 a-zA-Z0-9 \（）\,\.;\:\"\'!@$#*(\)-_?\&]+)$/i.test(value);
							},
							englishTest : function(value,element) {
