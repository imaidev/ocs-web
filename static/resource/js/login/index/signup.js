...
$(document).ready(function() {
				...
				company_email_code : {
					...
					remote : {
						...
						data : {
							...
							"emailNum" : function() {return $("#company_login_name").val();}
						}
					}
				},
		...
		$("#personal_mail_signup_form").validate({
			...
			messages : {
				...
				personal_email_password : {
					required : "请输入密码",
					minlength : "密码长度至少为6位",
					pswd_strength:"密码强度过低"
				},
				personal_email_password_confirm : {
					required : "请确认密码",
					minlength : "确认密码长度至少为6位",
					equalTo : "请确认密码与之前输入的密码一致"
				},
				personal_email_code : {
					required : "请输入验证码",
					number : "验证码应该为数字",
	...
	$("#get_company_email_code").on("click", function() {
		var email = $("#company_login_name").val();
		var validate = $("#company_login_name").valid();
		if (!validate) {
			return false;
		}
	...
	$("#company_email_submit").on("click", function() {
		...
		$("#company_email_signup_from").ajaxSubmit({
			...
			success : function(data) {
				if (data > 0) {
					// 激活邮箱界面
					window.location.href = activateEmailUrl1;
				} else if (data == '-1') {
					alert("注册失败，系统异常");
				} else if (data == '-2') {
					alert("用户名重复");
