...
//			1.01 - Fixed bug where unbinding would destroy all resize events
	...
	$.alerts = {
		...
		_reposition: function() {
			var topHeight = (($(window).height() / 2) - ($("#popup_container").outerHeight() / 2)) + $.alerts.verticalOffset;
			var leftWidth = (($(window).width() / 2) - ($("#popup_container").outerWidth() / 2)) + $.alerts.horizontalOffset-10;
			//ie浏览器下测试弹出的高度
			if($.browser.msie) { 
			   var nowIframe=window;
			   var scrollHeight=0;
			   var rangeTop=0;			   
			   if(nowIframe!=top){
			     while(nowIframe!=top){			   	
				     rangeTop=rangeTop+Math.floor($(nowIframe.parent.document).find("iframe:visible").offset().top);				  
				     scrollHeight=scrollHeight+Math.floor($(nowIframe).scrollTop());						   
				     nowIframe=nowIframe.parent;
			     };
		       topHeight = parseInt(Math.floor((top.document.documentElement.clientHeight-rangeTop - $("#popup_container").outerHeight())/2)  + scrollHeight+ $(window.top).scrollTop()-70);
			  }else{
				  topHeight=topHeight;
			  }
			}
