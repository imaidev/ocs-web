...
SWFUpload.prototype.initSettings = function (userSettings) {
	...
	if (!!this.settings.prevent_swf_caching) {
		var currurl = window.location.protocol + "//" + window.location.hostname + (window.location.port ? ":" + window.location.port : "");
		var indexSlash = window.location.pathname.lastIndexOf("/");
		if (indexSlash <= 0) {
		path = "/";
		} else {
			var indexTwo = window.location.pathname.indexOf("/",1);
			if(indexTwo <=0){
				path = window.location.pathname.substr(0, indexSlash) + "/";
			}else{
				path = window.location.pathname.substr(0, indexTwo) + "/";
			}
		}
		this.settings.flash_url = path + this.settings.flash_url + (this.settings.flash_url.indexOf("?") < 0 ? "?" : "&") + "preventswfcaching=" + new Date().getTime();
		this.settings.flash9_url = path + this.settings.flash9_url + (this.settings.flash9_url.indexOf("?") < 0 ? "?" : "&") + "preventswfcaching=" + new Date().getTime();
	}
	
	if (!this.settings.preserve_relative_urls) {
		this.settings.upload_url = SWFUpload.getRootURL(this.settings.upload_url);
		this.settings.button_image_url = SWFUpload.getAppURL(this.settings.button_image_url);
	}
	
	delete this.ensureDefault;
};
SWFUpload.getAppURL  = function (url) {
	try{
		var currurl = window.location.protocol + "//" + window.location.hostname + (window.location.port ? ":" + window.location.port : "");
		var indexSlash = window.location.pathname.lastIndexOf("/");
		if (indexSlash <= 0) {
...
SWFUpload.getRootURL  = function (url) {
	try{
