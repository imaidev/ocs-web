...
/******20150703--zxx鼠标放上去有拉帘的效果******/
function lalian(){
	var hoverLaLian=$('.hover_lalian');
	if(hoverLaLian){
		hoverLaLian.on('mouseenter',function(){
			selfA=$(this);
			$(this).find('.slide_left,.slide_right').stop().animate({'width':"50%"},500,function(){
				selfA.find('.lalian_chid').css({'display':'block'});
			});
		});
		hoverLaLian.on('mouseleave',function(){
			$(this).find('.slide_left,.slide_right').stop().animate({'width':"0%"},500);
			$(this).find('.lalian_chid').css({'display':'none'});
		});
	}
}
